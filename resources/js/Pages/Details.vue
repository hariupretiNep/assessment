<template>
    ...
    <HomeLayout class="mb-100">
            <div class="max-w-md mt-20 mb-100 bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl">
            <div class="md:flex">
                <div class="p-12">
                <div class="tracking-wide text-sm text-gray-500 font-semibold">Published at: {{details.publishedAt}}</div>
                <div v-if="details.author" class="tracking-wide text-sm text-gray-500 font-semibold ">Author: {{details.author}}</div>
                <div class="uppercase tracking-wide text-sm text-indigo-500 font-semibold mt-10">{{details.source.name}}</div>
            ...
            <div v-if="$page.props.auth.user" class="grid grid-cols-3 gap-4 relative p-10">
                <div class="block rounded p-2 bg-auto border-solid border-2 border-light-blue-500">
                    <button :disabled="(isLiked && isDisliked)" v-on:click="addLike()" class="text-gray-800 hover:text-blue-800 cursor-pointer">Like</button><span v-if="totalLike>0" v-bind="totalLike" class="text-gray-400 float-right">({{totalLike}})</span>
                </div>
                <div class="block rounded p-2 bg-auto border-solid border-2 border-light-blue-500">
                    <button :disabled="(isDisliked && isLiked)"  v-on:click="addDislike()" class="text-gray-800 hover:text-blue-800 cursor-pointer">Dislike</button><span v-if="totalDislike>0" class="text-gray-400 float-right">({{totalDislike}})</span>
                </div>
