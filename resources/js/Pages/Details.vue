    ...
    <HomeLayout>
            ...
            <div v-if="$page.props.auth.user" class="grid grid-cols-3 gap-4 relative p-10">
                <div class="block rounded p-2 bg-auto border-solid border-2 border-light-blue-500">
                    <button :disabled="isLiked" v-on:click="addLike()" class="text-gray-800 hover:text-blue-800 cursor-pointer">Like</button><span v-if="totalLike>0" v-bind="totalLike" class="text-gray-400 float-right">({{totalLike}})</span>
                </div>
                <div class="block rounded p-2 bg-auto border-solid border-2 border-light-blue-500">
                    <button :disabled="isDisliked"  v-on:click="addDislike()" class="text-gray-800 hover:text-blue-800 cursor-pointer">Dislike</button><span v-if="totalDislike>0" class="text-gray-400 float-right">({{totalDislike}})</span>
                </div>
